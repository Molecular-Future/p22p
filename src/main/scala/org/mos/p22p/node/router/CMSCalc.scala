...
object CMSCalc {
  ...
  def printMap(mapSets: DeepTreeSet, deep: Int = 0): Unit = {
    println(StringUtils.repeat("--", deep) + (mapSets.fromIdx+1));
    if (mapSets.treeHops.nodes.size > 0) {
      printSet(mapSets.treeHops, deep + 1);
    }
  }
  def printSet(mapSets: NodeSet, deep: Int = 0): Unit = {
    mapSets.nodes.map { n =>
      if (n.isInstanceOf[DeepTreeSet]) {
          printMap(n.asInstanceOf[DeepTreeSet],deep+1);
      }
    }
  }
